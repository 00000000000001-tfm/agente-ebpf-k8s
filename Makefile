...
$(SALIDA)/copy_fail.bpf.o: bpf/copy_fail.bpf.c bpf/vmlinux.h bpf/common.h | $(SALIDA)
	$(BPF_CLANG) $(BPF_CFLAGS) -c $< -o $@

$(SALIDA)/dns_exfil.bpf.o: bpf/dns_exfil.bpf.c bpf/vmlinux.h | $(SALIDA)
	$(BPF_CLANG) $(BPF_CFLAGS) -c $< -o $@

$(SALIDA)/cryptominer.bpf.o: bpf/cryptominer.bpf.c bpf/vmlinux.h bpf/common.h | $(SALIDA)
	$(BPF_CLANG) $(BPF_CFLAGS) -c $< -o $@

$(SALIDA)/agent: | $(SALIDA)
	CGO_ENABLED=0 GOOS=$(TARGETOS) GOARCH=$(TARGETARCH) go build -trimpath -ldflags='-s -w' -o $(SALIDA)/agent ./cmd/agent

programas: $(SALIDA)/monitor.bpf.o $(SALIDA)/privilege_escalation.bpf.o $(SALIDA)/reverse_shell_detector.bpf.o $(SALIDA)/copy_fail.bpf.o $(SALIDA)/dns_exfil.bpf.o $(SALIDA)/cryptominer.bpf.o $(SALIDA)/agent
