...
func loadGenericSensor(path string) (func(), *ebpf.Map, *ebpf.Map, error) {
	spec, err := ebpf.LoadCollectionSpec(path)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("Error al cargar eBPF: %v", err)
	}
	coll, err := ebpf.NewCollection(spec)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("Error en leer coleccion: %v", err)
	}
	var links []link.Link
	for name, prog := range coll.Programs {
		...
		if strings.HasPrefix(name, "kp_") {
			fnName := kprobeFunc(name)
			l, err = link.Kprobe(fnName, prog, nil)
		} else if strings.HasPrefix(name, "raw_") {
			l, err = link.AttachRawTracepoint(link.RawTracepointOptions{
				Name:    "sys_enter",
				Program: prog,
			})
		} else {
			l, err = link.Tracepoint("syscalls", progToTracepoint(name), prog, nil)
		}
		if err != nil {
			log.Printf("[warn] link %s: %v", name, err)
			continue
		}
		links = append(links, l)
	}
	wl := coll.Maps["watchlist"]
	rb := coll.Maps["unified_events"]
	cleanup := func() {
		for _, l := range links { l.Close() }
		coll.Close()
	}
	return cleanup, wl, rb, nil
}

func progToTracepoint(name string) string {
	// Mapea nombre de programa eBPF a nombre de tracepoint
	m := map[string]string{
		"cf_enter_socket": "sys_enter_socket",
		"cf_exit_socket":  "sys_exit_socket",
		"cf_bind":         "sys_enter_bind",
		"cf_sendmsg":      "sys_enter_sendmsg",
		"dns_rate_monitor": "sys_enter_sendmsg",
		"cm_enter_connect":"sys_enter_connect",
		"cm_exit_connect": "sys_exit_connect",
		"cm_execve":       "sys_enter_execve",
	}
	if tp, ok := m[name]; ok { return tp }
	log.Printf("[debug] progToTracepoint: no mapping for %q", name)
	return name
}

func kprobeFunc(name string) string {
	m := map[string]string{
		"kp_sys_socket": "__sys_socket",
		"kp_sys_bind":   "__sys_bind",
		"kp_sys_sendmsg": "__sys_sendmsg",
	}
	if fn, ok := m[name]; ok { return fn }
	// Eliminar prefijo kp_ y devolver el resto
	return strings.TrimPrefix(name, "kp_")
}

func isRawTracepoint(name string) bool {
	return strings.HasPrefix(name, "raw_")
}
...
func main() {
	...
	if s, err := startReverseShellSensor("/opt/agent/reverse_shell_detector.bpf.o"); err != nil {
        ...
        // Sensor Copy Fail (CVE-2026-31431)
        var genericWatchlists []*ebpf.Map

        if cfCleanup, wl, rbuf, err := loadGenericSensor("/opt/agent/copy_fail.bpf.o"); err != nil {
		log.Printf("[warn] sensor copy-fail: %v", err)
	} else {
		defer cfCleanup()
		if wl != nil { genericWatchlists = append(genericWatchlists, wl) }
		if rbuf != nil { go readGenericRing(rbuf, "copy-fail") }
		log.Printf("[agent] sensor copy-fail(CVE-2026-31431) cargado.")
	}

	if dnsCleanup, wl, rbuf, err := loadGenericSensor("/opt/agent/dns_exfil.bpf.o"); err != nil {
		log.Printf("[warn] sensor dns-exfil: %v", err)
	} else {
		defer dnsCleanup()
		if wl != nil { genericWatchlists = append(genericWatchlists, wl) }
	...
	if cmCleanup, wl, rbuf, err := loadGenericSensor("/opt/agent/cryptominer.bpf.o"); err != nil {
		log.Printf("[warn] sensor cryptominer: %v", err)
	} else {
		defer cmCleanup()
		if wl != nil { genericWatchlists = append(genericWatchlists, wl) }
	...
	startMetricsServer()
	startClusterMetricsLoop()
        gCorrelator = NewCorrelator()	//
	gKube = cs
	...
	if rs != nil && rs.wlMap != nil {
		log.Printf("[debug] rs.wlMap info: %v", rs.wlMap)
		watchlists = append(watchlists, rs.wlMap)
	}

	watchlists = append(watchlists, genericWatchlists...)
...
func handleIncidentLevel(ctx context.Context, ns, pod string, level int) error {
	...
	switch level {
	case LevelObserve:
		// Nivel 1: solo alertar, sin bloquear
		log.Printf("[WARN][L1] ACTIVIDAD SOSPECHOSA ns=%s pod=%s — observando sin bloquear", ns, pod)
		metricIncidentsTotal.WithLabelValues("L1").Inc()
		return nil

	case LevelQuarantine:
		// Nivel 2: cuarentena + Kyverno (comportamiento actual)
		log.Printf("[ALERT][L2] INCIDENTE CONFIRMADO ns=%s pod=%s — cuarentena", ns, pod)
		metricIncidentsTotal.WithLabelValues("L2").Inc()
		if err := gKM.TempQuarantineEgress(ctx, ns, pod, 15*time.Minute); err != nil {
			log.Printf("[warn] cuarentena red fallida: %v", err)
		}
		if err := gKM.UpdateBanlist(ctx, fp); err != nil {
			return fmt.Errorf("Lista de baneos: %w", err)
		}
		if err := gKM.CreatePreventionPolicy(ctx, fp); err != nil {
			return fmt.Errorf("Politica preventiva: %w", err)
		}
		if err := gKM.CreateCleanupForPod(ctx, ns, pod); err != nil {
			return fmt.Errorf("Politica de limpieza: %w", err)
		}
		log.Printf("[kyverno] politica de prevencion generada: ns=%s name=%s (images=%d sa=%q)",
			ns, pod, len(imgs), p.Spec.ServiceAccountName)

	case LevelKill:
		// Nivel 3: eliminación inmediata + cuarentena extendida
		log.Printf("[CRITICAL][L3] ATAQUE CRITICO ns=%s pod=%s — KILL inmediato", ns, pod)
		metricIncidentsTotal.WithLabelValues("L3").Inc()
		if err := gKM.TempQuarantineEgress(ctx, ns, pod, 60*time.Minute); err != nil {
			log.Printf("[warn] cuarentena red fallida: %v", err)
		}
		gracePeriod := int64(0)
		if err := gKube.CoreV1().Pods(ns).Delete(ctx, pod, metav1.DeleteOptions{
			GracePeriodSeconds: &gracePeriod,
		}); err != nil {
			log.Printf("[warn] eliminacion pod fallida: %v", err)
		} else {
			log.Printf("[L3] pod %s/%s eliminado inmediatamente", ns, pod)
		}
		if err := gKM.UpdateBanlist(ctx, fp); err != nil {
...
func (h *resourceHandler) sync(pod *corev1.Pod) {
	...
	for m := range ids {
			...
			// Actualizar watchlist del sensor RS directamente
			if h.rsSensor != nil {
				if err := h.rsSensor.addWatch(m); err != nil {
					log.Printf("[error] rs watchlist mntns=%d: %v", m, err)
				}
			}
		}
}
...
// readGenericRing lee eventos del ring buffer de sensores genéricos
func readGenericRing(m *ebpf.Map, sensorName string) {
	...
	for {
		...
		if len(rec.RawSample) < 8+4+4+1+1+16+32 {
			continue
		}
		b := rec.RawSample
		pid   := le32(b[8:12])
